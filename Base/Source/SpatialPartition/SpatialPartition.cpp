...
float CSpatialPartition::CalculateDistanceSquare(Vector3* theCameraPosition, const int xIndex, const int zIndex)
{
	...
	float zDistance = (zIndex * zNumOfGrid + (zSize / 2)) - theCameraPosition->z;*/
...
{
	LevelOfDetails_Distance[0] = distance_High2Mid;
	LevelOfDetails_Distance[1] = distance_Mid2low;
}

// Check if CGrid is visible to the camera
